...
public class MergeSortCoarsened {
    public static void main(String[] args) {
        ...
        int threshold = 10; // You can adjust the threshold for optimal performance.
    ...
    // Benchmarking loop for the optimized MergeSort.
    private static void runMergeSortCoarsened(
            int outerLoops, int[] exponents, Map<Integer, double[]> arrays,
            int threshold) {
        System.out.println(
                "Running MergeSort with coarsened base case (threshold = " + threshold +
                        ") for Benchmarking:");
    ...
    public static void mergesortCoarsened(
            double[] A, int p, int r, int k) {
        if (r - p + 1 <= k) {
    ...
    // Merge two sorted subarrays specific to this optimized merge sort.
    ...
    // Standard insertion sort for sorting small subarrays.
